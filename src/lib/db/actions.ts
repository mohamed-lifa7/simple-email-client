...
export async function sendEmail(formData: z.infer<typeof schema>) {
...
export async function createNewFolder(folderName: string) {
	...
	try {
		...
		let folderResult = await client.sql`
		INSERT INTO folders(name) VALUES(${folderName}) RETURNING id;
	  `;
		const folderId = folderResult.rows[0].id;
		await client.sql`
		INSERT INTO user_folders(user_id, folder_id) VALUES(${senderId}, ${folderId}) RETURNING id;
	  `;
